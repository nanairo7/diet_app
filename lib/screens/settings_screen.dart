...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  /// スロットのON/OFFを切り替える。
  /// 初めて有効化する場合（他にONのスロットがなければ）通知権限をリクエストする。
  Future<void> _toggleSlot(int index, bool enabled) async {
    if (enabled) {
      final anyEnabled = _slots.any((s) => s.enabled);
      if (!anyEnabled) {
    ...
    if (!mounted) return;
    await context.read<DietProvider>().saveNotificationSettings(slots: updated);
  }
  ...
  Widget _buildSlotTile(
    ...
    final timeLabel =
        TimeOfDay(hour: slot.hour, minute: slot.minute).format(context);
    return ListTile(
      leading: SizedBox(
        width: 32,
        child: Text(
